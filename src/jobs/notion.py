...
def build_create_properties(job: Job, run_date: str) -> dict:
    ...
    return {
        ...
        "Source Type": select(job.source_type),
        "Minimum Years Signal": text(job.min_years_signal),
        "Minimum / Basic Qualifications": text(job.minimum_qualifications),
        "Preferred Qualifications": text(job.preferred_qualifications),
